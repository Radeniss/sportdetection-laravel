    ...
    <table class="w-full">
        ...
        <tbody>
            @forelse ($videos as $video)
                <tr
                    class="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                    <td class="py-4 px-6">
                    ...
                    <td class="py-4 px-6 text-slate-600 dark:text-slate-300">
                        {{ $video->created_at->format('d M Y, H:i') }}</td>
                    <td class="py-4 px-6 text-center">
                        ...
                        @elseif($video->status == 'processing')
                            <span
                                class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-yellow-100 text-yellow-700">
                    ...
                    <td class="py-4 px-6 text-center">
                        <div class="flex items-center justify-center space-x-2">
                            @if ($video->status == 'completed' && $video->processed_filename)
                                <a href="{{ config('services.flask.url') }}/static/processed/{{ rawurlencode($video->processed_filename) }}"
                                    target="_blank"
                                    class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors">Lihat
                                    Hasil</a>
                                <a href="{{ config('services.flask.url') }}/api/download/{{ rawurlencode($video->processed_filename) }}"
                                    class="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors">Download</a>
                            @else
                                <button
                                    class="bg-gray-400 text-white font-semibold py-2 px-4 rounded-lg cursor-not-allowed"
                                    disabled>Lihat Hasil</button>
                            @endif

                            @if (in_array($video->status, ['pending', 'processing']))
                                <form action="{{ route('video.cancel', $video) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit"
                                        class="bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-2 px-4 rounded-lg transition-colors">Batalkan</button>
                                </form>
                            @endif
                            ...
                            <form action="{{ route('video.destroy', $video) }}" method="POST" class="inline"
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors">Hapus</button>
                            </form>
                        </div>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="4" class="text-center py-12">
                        <svg class="w-20 h-20 mx-auto text-slate-300 dark:text-slate-500 mb-4" fill="none"
