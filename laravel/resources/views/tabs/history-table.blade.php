        ...
        <tbody>
            <template x-for="(item, index) in historyData" :key="index">
                <tr class="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                    ...
                    <td class="py-4 px-6 text-center">
                        <button x-show="item.status === 'Selesai'" @click="alert('Melihat hasil: ' + item.name)"
                            class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors mr-2">Lihat
                            Hasil</button>
                        <button x-show="item.status === 'Selesai'" @click="alert('Mengunduh: ' + item.name)"
                            class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors">Unduh</button>
                        <button x-show="item.status === 'Gagal'" @click="alert('Mengulangi: ' + item.name)"
                            class="bg-slate-600 hover:bg-slate-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors">Coba
                            Lagi</button>
                    </td>
                </tr>
            </template>
