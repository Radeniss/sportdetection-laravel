                ...
                <tr class="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                    ...
                    <td class="py-4 px-6 text-center">
                        <div class="flex items-center justify-center space-x-2">
                            @if($video->status == 'completed')
                                <a href="{{ config('services.flask.url') }}/static/processed/{{ $video->processed_filename }}" target="_blank"
                                    class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors">Lihat Hasil</a>
                            @else
                            ...
                            @if(in_array($video->status, ['pending', 'processing']))
                                <form action="{{ route('video.cancel', $video) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-2 px-4 rounded-lg transition-colors">Batalkan</button>
                                </form>
                            @endif

                            <form action="{{ route('video.destroy', $video) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus video ini?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors">Hapus</button>
                            </form>
                        </div>
                    </td>
                </tr>
